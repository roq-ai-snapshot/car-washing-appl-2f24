...
import { useRoqClient, useServiceFindManyWithCount } from 'lib/roq';
...
export function ServiceListPage(props: ServiceListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useServiceFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.service.delete({ where: { id } });
      await mutate();
    } catch (error) {
